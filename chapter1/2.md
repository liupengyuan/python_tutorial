...
Then an integer M, and and then N lines follow. Each line contains 1 integer number. 
Print the letter SUM (uppercase) with a blank space before and after the equal signal followed by the sum of the N numbers,  andl etter PRODUCT (uppercase) with a blank space before and after the equal signal followed by the sum of the M integer numbers,  according to the following example.  
Samples Input：  
5  
10.0  
9.2  
...
PRODUCT = 48 
 - 练习5：  
Input contains many lines, each line contains 1 integer number and the last line is a single '%'.  
...
Samples Output:   
SUM = 30  
 - 练习6：用户可以输入的任意多个数字，并打印改数字，以及此时所有数字的和与积，直到: 输入所有数字的和>100，且所有数字的积<0。
