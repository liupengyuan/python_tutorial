...
对任意静态网页，均可以利用以上类似的方法，抓取感兴趣的内容。对更复杂的网页，需要对BeautifulSoap进一步进行了解，可参考其官方文档：`https://www.crummy.com/software/BeautifulSoup/bs4/doc/`，而有些网页很不规律，也不太容易用beautifulSoap对一些内容进行提取，这时，可以利用正则表达式进行模式匹配进行提取。
