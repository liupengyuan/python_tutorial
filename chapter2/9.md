...
9、利用字典保存超市商品，内容为各种商品的名称（不重复）、单价、库存。商品可添加十种左右，价格适中，库存随机分布在1-10中。你去这个超市购买商品，带现金为n，打印此时超市商品清单（1-10），你通过选择上述数字来进行商品购买，程序会给提示，你购买多少件，购买了各种商品加入你的购物车（可用字典模拟），最后结账，此时列出购物车内所有商品、单价、个数、总价，最后还剩多少钱，以及此时你购买后，商品及此时库存。请写程序模拟上述过程。  
