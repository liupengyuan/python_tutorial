...
0、编写一个递归函数，参数为一个字符串，实现字符串倒置打印。  
1、编写一个递归函数，参数为一个字符和一个字符串，统计该字符在字符串中出现的次数。    
2、编写一个递归函数，参数为n,k，求n^k的值（k>=0）。   
3、某的顶端有一个箱子，它在一个4个箱子组成的层面上，而这4个箱子又在一个9个箱子组成的层面上，以此类推。写一个递归函数，参数为层高，返回箱子总数。  
4、一堆麦粒中放养一群鸡，第一天吃了一半多一粒； 第二天又将剩下的麦粒吃掉一半多一粒。以后每天都吃了前一天剩下的一半多一个。问题：请问，到了第n天早上想再吃时，却发现只剩下一粒麦粒了。写一个递归函数，参数为n，函数返回这堆麦粒总数。  
5、随机生成100000个整数（1-100000之间），存入d:\temp\numbers.txt中，每行一个。读入该文件，将其利用内置的sort方法排序，并将排序结果存入到d:\temp\numbers_sorted.txt中。  
6、随机生成1000个整数（99000-100000之间）。打开d:\temp\numbers_sorted.txt并读入文件内容。a. 顺序查找这1000个整数是否在文件中出现;b. 用自写的递归二分查找这1000个整数是否在文件中出现。得到两者所用时间的差（秒）。  
7、统计文件：http://yunpan.blcu.edu.cn:80/link/DEB477A711E6328C9BF65D0BDBCAAD66 的字频  
8、随机生成1万个整数，范围在0-10万之间，求其中每个整数出现的次数。并按照整数大小排序输出整数及出现次数。  
9、利用字典保存超市商品，内容为各种商品的名称（不重复）、单价、库存。商品可添加十种左右，价格适中，库存随机分布在1-10中。你去这个超市购买商品，带现金为n，打印此时超市商品清单（1-10），你通过选择上述数字来进行商品购买，程序会给提示，你可以输入你购买这个商品多少件，这样，你会购买各种商品加入你的购物车（可用字典模拟），最后结账，此时列出购物车内所有商品、单价、个数、总价，最后还剩多少钱，以及此时你购买后，商品及此时库存。请写程序模拟上述过程。  
10、统计本任务中的`语料.txt`文件，得到每一个词对应不同词性的频次，可表示为：{词:[['n',1000], ['v', 98]...],...}，即在`dict`对象中，`key`为词，`value`为列表，输出词频最高的50个条目。
